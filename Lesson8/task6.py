...
в проекте «Склад оргтехники» максимум возможностей,
...
class DataProducts(ABC):
    ...
    def add_product(self, product, count=1):
        if isinstance(count, int) == False:
            raise ValueError('Указывайте количество числовым типом')
        ...
        elif isinstance(product, Scanner):
            self.scanners = self._sum_product(self.scanners, product, count)
        else:
            raise ValueError('Передаваемый продукт должен быть объектом класса OfficeEquipment')
